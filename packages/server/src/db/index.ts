...
export const updateUserSalt = async (userId: string, salt: string): Promise<void> => {
	await db.update(users).set({ salt, updatedAt: new Date() }).where(eq(users.id, userId));
};
...
export const updateUserRecoveryBlob = async (
	...
	await db.update(users).set({ recoveryBlob, updatedAt: new Date() }).where(eq(users.id, userId));
};
...
export const createOtpCode = async (email: string, code: string): Promise<OtpCode> => {
...
export const createSession = async (userId: string, refreshTokenHash: string): Promise<Session> => {
...
export const findValidSession = async (
	...
	const result = await db
		...
		.where(
			and(eq(sessions.refreshTokenHash, refreshTokenHash), gt(sessions.expiresAt, new Date())),
		);
	return result[0];
};
...
export const listUserBlobs = async (userId: string): Promise<Pick<Blob, "id" | "updatedAt">[]> => {
	return db
...
export const findBlobById = async (id: string, userId: string): Promise<Blob | undefined> => {
	const result = await db
		.select()
		.from(blobs)
...
export const deleteBlob = async (id: string, userId: string): Promise<boolean> => {
	const result = await db
		.delete(blobs)
		.where(and(eq(blobs.id, id), eq(blobs.userId, userId)))
...
export const countUserBlobs = async (userId: string): Promise<number> => {
	const result = await db.select({ count: count() }).from(blobs).where(eq(blobs.userId, userId));
...
export const findSkillById = async (id: string, userId: string): Promise<Skill | undefined> => {
	const result = await db
		.select()
		.from(skills)
...
export const createSkill = async (userId: string, skillKey: string): Promise<Skill> => {
	const result = await db.insert(skills).values({ userId, skillKey }).returning();
	return result[0]!;
};

export const updateSkillCurrentHash = async (skillId: string, hash: string): Promise<void> => {
	await db
		.update(skills)
...
export const findSkillVersion = async (
	...
	const result = await db
		.select()
		.from(skillVersions)
		.where(and(eq(skillVersions.skillId, skillId), eq(skillVersions.hash, hash)));
	return result[0];
};

export const listSkillVersions = async (skillId: string, limit = 50): Promise<SkillVersion[]> => {
	return db
		.select()
		.from(skillVersions)
...
export const getUserKeyPair = async (
	...
	const row = result[0];
	if (!row?.publicKey || !row?.encryptedPrivateKey || !row?.privateKeyIv || !row?.privateKeyTag) {
		return null;
	}
...
export const createTeam = async (name: string, ownerId: string): Promise<Team> => {
	const result = await db.insert(teams).values({ name, ownerId }).returning();
	return result[0]!;
};
...
export const listUserTeams = async (userId: string): Promise<TeamWithMemberInfo[]> => {
	...
	for (const team of memberTeams) {
		const [memberCountResult, skillCountResult] = await Promise.all([
			...
			db.select({ count: count() }).from(teamSkills).where(eq(teamSkills.teamId, team.id)),
		]);
...
export const addTeamMember = async (
	...
	const result = await db.insert(teamMembers).values({ teamId, userId, role, status }).returning();
	return result[0]!;
};
...
export const getTeamActiveKeyVersion = async (teamId: string): Promise<number | null> => {
	const result = await db
...
export const rotateTeamKeyAndRemoveMember = async (
	...
	await db.transaction(async (tx) => {
		await tx
			.delete(teamMembers)
			.where(and(eq(teamMembers.teamId, teamId), eq(teamMembers.userId, removedUserId)));
		...
		for (const envelope of envelopes) {
			...
			await tx
				...
				.where(
					and(eq(teamMembers.teamId, teamId), eq(teamMembers.userId, envelope.recipientUserId)),
				);
		}
	});
};
...
export const listTeamMembers = async (teamId: string): Promise<TeamMemberWithEmail[]> => {
	const result = await db
		.select({
...
export const listPendingTeamMembers = async (teamId: string): Promise<TeamMemberWithEmail[]> => {
	const result = await db
		.select({
			teamId: teamMembers.teamId,
			userId: teamMembers.userId,
...
export const markTeamInviteUsed = async (inviteId: string): Promise<void> => {
	await db.update(teamInvites).set({ usedAt: new Date() }).where(eq(teamInvites.id, inviteId));
};

export const listTeamInvites = async (teamId: string): Promise<TeamInvite[]> => {
	return db
		.select()
		.from(teamInvites)
...
export const createTeamSkill = async (
	...
	const result = await db.insert(teamSkills).values({ teamId, skillKey, createdBy }).returning();
	return result[0]!;
};
...
export const findTeamSkillByKey = async (
	...
	const result = await db
		.select()
		.from(teamSkills)
		.where(and(eq(teamSkills.teamId, teamId), eq(teamSkills.skillKey, skillKey)));
	return result[0];
};
...
export const updateTeamSkillCurrentHash = async (skillId: string, hash: string): Promise<void> => {
	await db
		.update(teamSkills)
		.set({ currentHash: hash, updatedAt: new Date() })
...
export const findPublicSkillById = async (id: string): Promise<PublicSkill | undefined> => {
	const result = await db.select().from(publicSkills).where(eq(publicSkills.id, id));
	return result[0];
};
...
export const listApprovedPublicSkills = async (
	...
	(Pick<
		PublicSkill,
		"id" | "slug" | "name" | "description" | "category" | "tags" | "downloadCount" | "publishedAt"
	> & { authorEmail: string })[]
> => {
	const baseQuery = db
		.select({
		...
		.where(
			category
				? and(eq(publicSkills.status, "approved"), eq(publicSkills.category, category))
...
export const listUserPublicSkills = async (userId: string): Promise<PublicSkill[]> => {
	return db
		.select()
		.from(publicSkills)
...
export const incrementPublicSkillDownloads = async (id: string): Promise<void> => {
	await db
		.update(publicSkills)
		.set({
...
export const deletePublicSkill = async (id: string, userId: string): Promise<boolean> => {
	const result = await db
		.delete(publicSkills)
...
export const countUserPendingPublicSkills = async (userId: string): Promise<number> => {
	const result = await db
		.select({ count: count() })
		.from(publicSkills)
		.where(and(eq(publicSkills.userId, userId), eq(publicSkills.status, "pending")));
	return result[0]?.count ?? 0;
};
...
export const findTeamSkillVersion = async (
	...
	const result = await db
		...
		.where(and(eq(teamSkillVersions.skillId, skillId), eq(teamSkillVersions.hash, hash)));
	return result[0];
};
