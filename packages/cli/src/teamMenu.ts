...
import { getTeam, listTeams, type TeamListItem } from "./api.js";
import {
	runTeamAccept,
	runTeamCreate,
	runTeamInvite,
 ...
 */
const runTeamActions = async (team: TeamListItem, cache: KeyCache): Promise<"back" | "menu"> => {
	while (true) {
		...
		if (canWrite) {
			...
			options.push({ value: "pull", label: "Pull", hint: "pull skills from team" });
		}
		...
		if (canManage) {
			options.push({ value: "invite", label: "Invite member", hint: "add someone to this team" });
		}
