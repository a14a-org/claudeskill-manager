...
export const saveTeamSyncIndex = async (teamSlug: string, index: TeamSyncIndex): Promise<void> => {
...
export const pushTeamSkills = async (params: {
	...
	const filtered = params.skillFilter
		? allSkills.filter(
				(skill) =>
					skill.name === params.skillFilter || `${skill.type}:${skill.name}` === params.skillFilter,
			)
		: allSkills;
...
export const pullTeamSkills = async (params: {
	...
	if (!listResult.ok) {
		return {
			pulled: 0,
			errors: [`Failed to list team skills: ${listResult.error}`],
			conflicts: [],
		};
	}
