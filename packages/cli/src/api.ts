 ...
 */
const apiRequest = async <T>(path: string, options: RequestInit = {}): Promise<ApiResponse<T>> => {
...
export const requestOtp = async (email: string): Promise<ApiResponse<OtpRequestResponse>> => {
	...
	try {
		...
		if (!response.ok) {
			return {
				ok: false,
				error: (data as { error?: string }).error ?? "Failed to send OTP",
				status: response.status,
			};
		}
...
export const verifyOtp = async (
	...
	try {
		...
		if (!response.ok) {
			return {
				ok: false,
				error: (data as { error?: string }).error ?? "Invalid code",
				status: response.status,
			};
		}
