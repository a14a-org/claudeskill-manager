 ...
 */
const renderDependencyTree = (skills: Skill[], graph: Map<string, DependencyNode>) => {
	console.log("");
	...
	if (pipelines.length > 0) {
		...
		pipelines.forEach((pipeline, i) => {
			...
			const prefix = isLast ? "│   └── " : "│   ├── ";
			console.log(`${prefix}${pipeline.from.name} → ${pipeline.to.name}`);
			console.log(
	...
	if (hasStandalone) {
		...
		types.forEach((type, typeIdx) => {
			...
			const isLastType = remainingTypes.every((t) => standaloneByType[t].length === 0);
 ...
 */
const renderToolMatrix = (skills: Skill[], graph: Map<string, DependencyNode>) => {
	console.log("");
	...
	const nameWidth = Math.max(20, ...skills.map((s) => s.name.length + s.type.length + 3));
	const toolWidth = 6;

	// Header
	const header = "Name".padEnd(nameWidth) + tools.map((t) => t.padStart(toolWidth)).join("");
	console.log(header);
	...
	sorted.forEach((skill) => {
		...
		const cells = tools.map((t) => (skillTools.has(t) ? "✓" : "·").padStart(toolWidth)).join("");
...
const renderDefaultList = (skills: Skill[]) => {
	...
	types.forEach((type, typeIndex) => {
		...
		const isLastType = types.slice(typeIndex + 1).every((t) => byType[t].length === 0);
		const typePrefix = isLastType ? "└── " : "├── ";
		const typeChildPrefix = isLastType ? "    " : "│   ";
		...
		items.forEach((skill, index) => {
			...
			if (hasFiles) {
				console.log(`${typeChildPrefix}${childPrefix}├── Files: ${skill.files!.length + 1}`);
			}

			// Last modified
			console.log(`${typeChildPrefix}${childPrefix}├── Modified: ${relativeTime}`);
