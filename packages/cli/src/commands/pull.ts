...
import { getSkillKey, listAllSkills } from "@claudeskill/core";
import * as api from "../api.js";
import { loadConfig } from "../config.js";
import { loadCredentials } from "../credentials.js";
import { getMasterKey, loadSyncIndex, pullSkills } from "../sync.js";
...
export const runPull = async (options: PullOptions = {}) => {
	...
	if (options.interactive) {
		...
		const changes = listResult.data.skills
			...
			.filter(
				(change): change is { skillKey: string; status: "new" | "updated" } => change !== null,
			);
