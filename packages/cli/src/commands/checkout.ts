...
import { decryptSkill, getMasterKey, loadSyncIndex, saveSyncIndex } from "../sync.js";
...
export const runCheckout = async (skillKey: string, hash: string) => {
	...
	let decrypted;
	try {
		decrypted = decryptSkill(result.data.encryptedData, result.data.iv, result.data.tag, masterKey);
	} catch (err) {
		spinner.stop("Failed");
	...
	try {
		if (skillType === "skill" && decrypted.files) {
			...
			await Promise.all(
				decrypted.files.map((f) => writeFile(join(skillDir, f.name), f.content, "utf-8")),
			);
		} else {
