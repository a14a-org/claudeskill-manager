...
import { computeSkillHash, getSkillKey, listAllSkills, type SkillType } from "@claudeskill/core";
import * as api from "../api.js";
import { loadConfig } from "../config.js";
import { loadCredentials } from "../credentials.js";
import { getMasterKey, loadSyncIndex, pushSkills } from "../sync.js";
...
export const runPush = async (options: PushOptions = {}) => {
	...
	if (options.interactive) {
		...
		const changes = skills
			...
			.filter(
				(change): change is { name: string; type: SkillType; status: "new" | "modified" } =>
					change !== null,
			);
	...
	const { pushed, errors } = await pushSkills(
		masterKey,
		(msg) => {
			spinner.message(msg);
		},
