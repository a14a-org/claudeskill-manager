 ...
 */
export const parseFrontmatter = (content: string): { metadata: SkillMetadata; body: string } => {
 ...
 */
export const serializeFrontmatter = (metadata: SkillMetadata, body: string): string => {
 ...
 */
export const readSkill = async (filePath: string, type: SkillType = "command"): Promise<Skill> => {
...
export const readDirectorySkill = async (
	...
	try {
		...
		const mainFile = entries.find((e) => {
			...
			return lower === "skill.md" || lower === `${name.toLowerCase()}.md` || lower === "index.md";
		});
...
export const listDirectorySkills = async (
	...
	try {
		...
		const skillPromises = dirEntries.map(async ({ path: dirPath }) => {
 ...
 */
export const detectImplicitDependencies = (skill: Skill, allSkillNames: string[]): string[] => {
 ...
 */
export const buildDependencyGraph = (skills: Skill[]): Map<string, DependencyNode> => {
	...
	skills.forEach((skill) => {
		...
		graph.set(skill.name, {
			...
			model: typeof skill.metadata["model"] === "string" ? skill.metadata["model"] : null,
		});
	});
